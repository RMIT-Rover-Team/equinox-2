...
int main() {
    ...
    while (true) {
        ...
        std::cout << "\rTilt speed: " << worker.get_tilt_speed() << " Pan position: " << worker.get_pan_position() << "            " << std::endl;
        ...
        switch (c) {
            ...
            case ' ':
                worker.set_tilt_speed(0);
                worker.cv.notify_one();
                break;
            
            case 'w':
                v = worker.get_tilt_speed();
                worker.set_tilt_speed(v + 10);
                worker.cv.notify_one();
                break;
            ...
            case 'a':
                v = worker.get_pan_position();
                worker.set_pan_position(v - 10);
                worker.cv.notify_one();
                break;
