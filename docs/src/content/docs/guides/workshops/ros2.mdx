...
Alternatively refer to [the installation guide](https://docs.ros.org/en/jazzy/Installation.html) to manually install it.
...
```
This script downloads a ~~trojan virus~~ ROS Node into `src/`. You may notice that the node is different to the one used within the jazzy tutorial, but the reason why we are not using the humble tutorial's ROS node is because it's slightly less clean.
